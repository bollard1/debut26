// Initialize infinite scroll immediately when script loads
(function initInfiniteScroll() {
  // Early exit if not a collection page
  if (!window.location.pathname.includes('/collections/')) return;
  
  // Use interactive ready state instead of DOMContentLoaded for faster execution
  function initWhenReady() {
    const productGrid = document.querySelector('[data-infinite-scroll]');
    const paginationInfo = document.getElementById('pagination-info');
    const loader = document.getElementById('infinite-scroll-loader');
    
    // Only proceed if all required elements exist
    if (!productGrid || !paginationInfo) return;
    
    console.log('Initializing infinite scroll...');
    let isLoading = false;
    let currentPage = parseInt(paginationInfo.dataset.currentPage);
    const totalPages = parseInt(paginationInfo.dataset.pages);
    ...
    const loadMoreProducts = async () => {
      ...
      try {
        ...
        let nextUrl = new URL(paginationInfo.dataset.nextUrl, window.location.origin);
        ...
        // Update pagination info for next load FIRST
        const newPaginationInfo = doc.getElementById('pagination-info');
        const hasMorePages = newPaginationInfo && newPaginationInfo.dataset.nextUrl;
        
        // CRITICAL: Handle availability filtering
        if (newProducts.length === 0) {
          console.log('No available products found on this page');
          
          // If no more pages exist, stop completely
          if (!hasMorePages) {
            console.log('No more pages exist, stopping infinite scroll');
            if (loader) {
              loader.style.display = 'none';
              observer.unobserve(loader); // Stop observing completely
            }
            isLoading = false;
            return;
          }
          
          // Continue to next page - might have available products
          console.log('Continuing to next page - might have available products');
          if (newPaginationInfo) {
            paginationInfo.dataset.nextUrl = newPaginationInfo.dataset.nextUrl;
          }
          currentPage++;
          isLoading = false; // Reset loading state before recursion
          
          // Immediately try to load next page
          setTimeout(() => loadMoreProducts(), 100);
          return;
        }
        
        // Update pagination for next load if products were found
        if (hasMorePages) {
          paginationInfo.dataset.nextUrl = newPaginationInfo.dataset.nextUrl;
        }
        ...
        // Hide loader if we're on the last page or reached total pages
        if (currentPage >= totalPages || !hasMorePages) {
          console.log('Reached end of collection, stopping infinite scroll');
          if (loader) {
            loader.style.display = 'none';
            observer.unobserve(loader); // Stop observing completely
          }
    ...
    // Modified Intersection Observer with better threshold
  ...
  // Smart DOM ready detection - faster than DOMContentLoaded
  ...
  Cart.prototype = Object.assign({}, Cart.prototype, {
    ...
    _updateItemQuantity: function(
      ...
      fetch('/cart/change.js', request)
        ...
        .then(
          function(state) {
            // Check if Shopify returned an error (e.g., insufficient stock)
            if (state.status && state.status >= 400) {
              // Show error message on the item
              var errorMessage = state.description || state.message || theme.strings.cartError;
              itemElement.querySelectorAll(selectors.cartQuantityErrorMessage).forEach(function(el) {
                el.textContent = errorMessage;
              });
              itemElement.querySelectorAll(selectors.cartQuantityErrorMessageWrapper).forEach(function(el) {
                el.classList.remove(classes.hide);
              });
              itemElement.querySelectorAll(selectors.inputQty).forEach(function(el) {
                el.classList.add(classes.inputError);
              });
              return;
            }
    ...
    _createCart: function(state) {
      ...
      // Calculate cart-level discount total from the discount applications
      // (fixes timing issue where state.total_price isn't updated immediately after item removal)
      var cartLevelDiscountTotal = (state.cart_level_discount_applications || []).reduce(function(sum, discount) {
        return sum + (discount.amount || 0);
      }, 0);

      // Use items_subtotal_price minus cart-level discounts for accurate subtotal
      var calculatedSubtotal = (state.items_subtotal_price || 0) - cartLevelDiscountTotal;

      /* bgn_remove: Cart subtotal с BGN - При премахване: замени с theme.Currency.formatMoney(calculatedSubtotal, theme.moneyFormat) */
    ...
    _createCartDiscountList: function(cart) {
      ...
      return discounts
        .filter(function(discount) {
          // Данните са вложени в discount_application
          return discount && discount.discount_application && discount.discount_application.title;
        })
        .map(function(discount) {
          // Създаване на контейнер за отстъпката динамично
          var discountNode = document.createElement('div');
          discountNode.className = 'order-discount-card-wrapper';
          discountNode.setAttribute('data-cart-discount', '');

          // Вземи title и amount от вложения обект discount_application
          var discountTitle = discount.discount_application.title;
          var discountAmount = discount.amount || discount.discount_application.total_allocated_amount || 0;

          // Създаване на заглавие
          var titleSpan = document.createElement('span');
          titleSpan.className = 'order-discount order-discount--title order-discount--cart';
          titleSpan.innerHTML = '<span class="visually-hidden">Отстъпка:</span>' +
            '<span data-cart-discount-title>' + discountTitle + '</span>';

          // Създаване на сума на отстъпката
          var amountSpan = document.createElement('span');
          amountSpan.className = 'order-discount order-discount--cart order-discount--cart-total';
          /* bgn_remove: Cart discount amount с BGN - При премахване: замени _formatPriceWithBGN с theme.Currency.formatMoney */
          amountSpan.innerHTML = '-<span data-cart-discount-amount>' +
            this._formatPriceWithBGN(discountAmount) + '</span>';

          discountNode.appendChild(titleSpan);
          discountNode.appendChild(amountSpan);
    ...
    /* bgn_remove: ЦЯЛА ФУНКЦИЯ - Конвертиране EUR към BGN
       При премахване: изтрий цялата функция и замени всички извиквания на
       this._formatPriceWithBGN(price) с theme.Currency.formatMoney(price, theme.moneyFormat) */
    _formatPriceWithBGN: function(price) {
      var bgnRate = 1.95583;
      var eurFormatted = theme.Currency.formatMoney(price, theme.moneyFormat);
      var numPrice = Number(price) || 0;
      var bgnValue = Math.round(numPrice * bgnRate / 100 * 100) / 100;
      return eurFormatted + ' (' + bgnValue.toFixed(2) + ' лв.)';
    },

    _createItemPrice: function(original_price, final_price) {
      /* bgn_remove: Original price с BGN - При премахване: замени с theme.Currency.formatMoney(original_price, theme.moneyFormat) */
      var originalPriceHTML = this._formatPriceWithBGN(original_price);
      ...
      if (original_price !== final_price) {
        ...
        /* bgn_remove: Final price с BGN - При премахване: замени с theme.Currency.formatMoney(final_price, theme.moneyFormat) */
        resultHTML.querySelector(
          selectors.cartItemFinalPrice
        ).innerHTML = this._formatPriceWithBGN(final_price);
      } else {
        resultHTML = this.itemPriceListTemplate
    ...
    _createUnitPrice: function(unitPrice, unitPriceMeasurement) {
      ...
      /* bgn_remove: Unit price с BGN - При премахване: замени с theme.Currency.formatMoney(unitPrice, theme.moneyFormat) */
      unitPriceGroup.querySelector(
        selectors.unitPrice
      ).textContent = this._formatPriceWithBGN(unitPrice);
    ...
    _createItemDiscountList: function(item) {
      return item.line_level_discount_allocations.map(
        function(discount) {
          ...
          /* bgn_remove: Item discount amount с BGN - При премахване: замени с theme.Currency.formatMoney(discount.amount, theme.moneyFormat) */
          discountNode.querySelector(
            selectors.cartItemDiscountAmount
          ).textContent = this._formatPriceWithBGN(discount.amount);
